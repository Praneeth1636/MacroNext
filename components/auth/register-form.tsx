...
export function RegisterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (list?.length) {
        const first = list[0];
        if (first.meta?.paramName === "email_address")
          setFieldErrors((prev) => ({ ...prev, email: first.message }));
        else if (first.meta?.paramName === "password")
          setFieldErrors((prev) => ({ ...prev, password: first.message }));
    ...
    return (
      <AuthCard>
        <AuthCardHeader>
          ...
          <AuthSubtitle>We sent a verification code to your email.</AuthSubtitle>
        </AuthCardHeader>
  ...
  return (
    <AuthCard>
      <AuthCardHeader>
        ...
        <AuthSubtitle>Let&apos;s get you started</AuthSubtitle>
      </AuthCardHeader>
      <AuthSocialSection>
        <AuthButtonOutline
          type="button"
          onClick={() => signUp?.authenticateWithRedirect({ strategy: "oauth_google", redirectUrl: REDIRECT, redirectUrlComplete: REDIRECT })}
          disabled={!isLoaded}
        >
        ...
        <AuthButtonOutline
          type="button"
          onClick={() => signUp?.authenticateWithRedirect({ strategy: "oauth_github", redirectUrl: REDIRECT, redirectUrlComplete: REDIRECT })}
          disabled={!isLoaded}
        >
